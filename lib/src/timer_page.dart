    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (_) => TimerBloc(
              ticker: const Ticker(),
              sliderCubit: _sliderCubit,
              checkboxCubit: _checkboxCubit),
        ),
        BlocProvider(
...
class _TimerViewState extends State<TimerView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(title: const Center(child: Text('Timer app'))),
      body: Stack(
        children: [
          const Background(),
          Column(
            ...
            children: <Widget>[
              ...
              BlocBuilder<TimerBloc, TimerState>(
                builder: (context, state) {
                  return Visibility(
                      visible: state is! TimerRunInProgress,
                      child: Column(
                        children: [
                          SizedBox(
                            width: 180,
                            child: CheckboxListTile(
                              value: context.read<CheckboxCubit>().state.isChecked,
                              title: Text(
                                'counting up',
                          ...
                          Visibility(
                            ...
                            child: Slider(
                              value: context.read<SliderCubit>().state.value,
                              max: 300,
                              divisions: 10,
                              label: context
                                  .read<SliderCubit>()
